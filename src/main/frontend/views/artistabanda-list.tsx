...
import { AlbumService, ArtistaBandaService, ArtistaService, BandaService } from 'Frontend/generated/endpoints';
...
export default function ArtistaBandaListView() {
  ...
  useEffect(() => {
    ArtistaBandaService.listAll().then((data) => {
      setItems(data);
    });
  }, []);
  ...
  const order = (event, columnId) => {
    ...
    ArtistaBandaService.order (columnId, dir).then((data) => {
      setItems(data);
    });
  };

 /* const dataProvider = useDataProvider({
    list: () => ArtistaBandaService.listAll(),
  });*/
  ...
  const search = async () => {
    try {
      ...
      ArtistaBandaService.search(criterio.value, texto.value, 1).then(function (data) {
        setItems(data);
      });
  ...
  return (
    <main className="w-full h-full flex flex-col box-border gap-s p-m">
      ...
      <Grid items={items}>
        ...
        <GridSortColumn onDirectionChanged={(e) => order(e, "banda")} path="banda" header="Banda"/> 
        <GridSortColumn onDirectionChanged={(e) => order(e, "rol")} path="rol" header="Rol"/>
